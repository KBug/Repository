...
history_path = os.path.join(xbmc.translatePath('special://home/addons/plugin.video.prehrajto'),'resources', 'history.txt')
qr_path = os.path.join(xbmc.translatePath('special://home/addons/plugin.video.prehrajto'),'resources', 'qr.png')
...
def get_premium():
    if addon.getSetting("email") != '':
        login = {"password": addon.getSetting("password"),"email": addon.getSetting("email"), '_submit': 'Přihlásit+se', 'remember': 'on', 'do': 'login-loginForm-submit'}
...
def get_link(gl):
    ...
    file1 = json.loads(json.dumps(data))[-1]["file"]
    try:
        pattern = re.compile('.*var tracks = (.*?);.*', re.DOTALL)
        script = soup.find("script", text=pattern).string
        data = hjson.loads(pattern.findall(script)[0])
        file2 = json.loads(json.dumps(data))[0]["file"]
    except:
        file2 = ""
    return file1, file2
...
def search(name):
    ...
    if os.path.exists(history_path):
        lh = open(history_path, "r", encoding="utf-8").read().splitlines()
        if q not in lh:
            if len(lh) == 10:
                del lh[-1]
            lh.insert(0, q)
    ...
    while True:
        ...
        title = soup.find_all('h2',attrs={'class': 'video-item-title'})
        size = soup.find_all('strong',attrs={'class': 'video-item-info-size'})
        time = soup.find_all('strong',attrs={'class': 'video-item-info-time'})
        link = soup.find_all('a',{'class': 'video-item video-item-link'})
        next = soup.find_all('div',{'class': 'pagination-more'})
        for t,s,l,m in zip(title,size,link,time):
            videos.append((t.text , " (" + s.text[1:-1] + " - " + m.text[1:-1] + ")", 'https://prehraj.to:443' + l['href']))
        p = p + 1
...
def movie_category():
    name_list = [("Nejlépe hodnocené", "listing_tmdb_movie", "1", "top_rated"), ("Oblíbené", "listing_tmdb_movie", "1", "popular"), ("Novinky", "listing_tmdb_movie", "1", "upcoming"), ("Žánry", "listing_genre_category", "movie", ""), ("Rok", "listing_year_category", "movie", "")]
    for category in name_list:
...
def serie_category():
    name_list = [("Nejlépe hodnocené", "listing_tmdb_serie", "1", "top_rated"), ("Oblíbené", "listing_tmdb_serie", "1", "popular"), ("Vysílané", "listing_tmdb_serie", "1", "on_the_air"), ("Žánry", "listing_genre_category", "tv", ""),("Rok", "listing_year_category", "tv", "")]
    for category in name_list:
        list_item = xbmcgui.ListItem(label=category[0])
        url = get_url(action=category[1], name = category[2], type =category[3])
        xbmcplugin.addDirectoryItem(_handle, url, list_item, True)
